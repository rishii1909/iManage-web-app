    ...
    try {
        await TeamModel.findById({_id : team_id}, (err, response) => {
            ...
            if(!is_root(root_user, user_id)){
                return res.json(handle_error("Only the root user can add new billing admins."))
            }
            // Actual operations.
            users = users.filter(user => !team.billing_admins.includes(user))
            ...
            (err) => {
                if(err){
                    ...
                    return res.json(handle_success("Selected users are now a billing admin!"))
                }
            });
        });
    } catch (err) {
...
router.post('/revoke', async (req, res, next) => {
    ...
    try {
        await TeamModel.findById({_id : team_id}, (err, response) => {
            ...
            // Checks.
            if(!is_root(root_user, user_id)){
                return res.json(handle_error("Only the root user can revoke billing admin permissions."))
            }
            // Actual operations.
            TeamModel.updateOne({
                _id: team_id
            ...
            (err) => {
                if(err){
                    ...
                    return res.json(handle_success("Billing admin privileges have been revoked for selected users."))
                }
            });
